...
<?php
	$type =$_REQUEST['type'];    
    // selecting rows
	$sql = "SELECT * FROM labour where type='$type' ";
    if( isset($_GET['char']) ){
...
<?php
	...
	$sql = "select DISTINCT LEFT(name , 1) as firstCharacter,( select count(*) FROM labour WHERE type='$type' AND LEFT(name , 1)= firstCharacter ) AS counter FROM labour WHERE type='$type'";
...
?>
	...
	<a rel='facebox' href="labour_modify.php?id=<?=$row['id']?>" class="btn btn-warning btn-sm">Modify</a>
	<a rel='facebox' href="labour_delete.php?id=<?=$row['id']?>" class="btn btn-danger btn-sm">Delete</a>
	</td>
	</tr>
<?php
}
