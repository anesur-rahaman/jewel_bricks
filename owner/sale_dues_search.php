...
if(isset($_POST['query'])){
	$sale_dues_search=$_POST['query'];
	$stmt = $conn->prepare("SELECT * FROM customers WHERE name LIKE CONCAT(?, '%') OR address LIKE CONCAT(?, '%') OR phone LIKE CONCAT(?, '%') ORDER BY name");
	$stmt->bind_param("sss", $sale_dues_search, $sale_dues_search, $sale_dues_search);
}
else{
