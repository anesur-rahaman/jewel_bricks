...
{
	$balance = $row['sum_advance']-$row['sum_wages'];
}
?>
...
<td><?= $row['description'] ?> (<?= $row['month'] ?>, <?= $row['year'] ?>)</td>
...
<?php
 //echo "<tr>";
  ...
  $Color = "red";
  $result_running_advance = mysqli_query($conn, "SELECT SUM(advance) AS sum_advance, SUM(wages) AS sum_wages FROM staff_details where staff_id='$id' AND date <= '$temp_date' ORDER BY id DESC Limit $start, $perpage");
...
?>
<td>
<button><a rel='facebox' href="staff_details_modify.php?id=<?=$row['id']?>&staff_id=<?=$row['staff_id']?>" <i class="far fa fa-address-card-o"></i>Modify</a></button>
<button><a rel='facebox' href="staff_details_delete.php?id=<?=$row['id']?>&staff_id=<?=$row['staff_id']?>" <i class="far fa fa-address-card-o"></i><font size="" color="red">Delete</font></a></button>
</td>
<?php
  echo "</tr>";
}
