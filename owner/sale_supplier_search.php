...
if(isset($_POST['query'])){
	$sale_supplier_search=$_POST['query'];
	$stmt = $conn->prepare("SELECT * FROM customers WHERE type='Supplier' AND name LIKE CONCAT(?, '%') OR address LIKE CONCAT(?, '%') OR phone LIKE CONCAT(?, '%') ");
	$stmt->bind_param("sss", $sale_supplier_search, $sale_supplier_search, $sale_supplier_search);
}
else{
	$stmt = $conn->prepare("SELECT * FROM customers WHERE type='Supplier' ");	
}
	$stmt->execute();	
