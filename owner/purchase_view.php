...
$calc = $perpage * $page;
$start = $calc - $perpage;
$result = mysqli_query($conn, "SELECT * FROM purchase where vendor_id='$id' ORDER BY id DESC Limit $start, $perpage");
...
<?php
  ...
  <?php 
  $Color = "red";
  $result_running_due = mysqli_query($conn, "SELECT SUM(bill) AS sum_bill, SUM(payment) AS sum_payment FROM purchase where vendor_id='$id' AND date <= '$temp_date' ORDER BY id DESC Limit $start, $perpage");
  ...
  echo "<td><button><a href='purchase_bill.php?id=".$row['id']."&vendor_id=".$row['vendor_id']."'>Purchase Bill</a></button></td>" ;
  //echo "<td><button><a href='invoice.php?id=".$row['id']."&vendor_id=".$row['vendor_id']."'>Invoice</a></button></td>";?>
...
<?php
	if(isset($page))
	{
	$result = mysqli_query($conn,"select Count(*) As Total from bill where vendor_id='$id'");
	$rows = mysqli_num_rows($result);
	if($rows)
	{
	...
	{
	$j = $page - 1;
	echo "<span>&nbsp;<button><a id='page_a_link' href='purchase_view.php?page=$j&id=$id'>< Prev</a>&nbsp;</button></span>";
	}
	for($i=1; $i <= $totalPages; $i++)
	{
	if($i<>$page)
	{
	echo "<span>&nbsp;<button><a id='page_a_link' href='purchase_view.php?page=$i&id=$id'>$i</a>&nbsp;</button></span>";
	}
	else
	{
	...
	{
	$j = $page + 1;
	echo "<span>&nbsp;<button><a id='page_a_link' href='purchase_view.php?page=$j&id=$id'>Next</a></button></span>";
	}
	}
?></td>
<td></td>
