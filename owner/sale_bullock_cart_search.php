...
if(isset($_POST['query'])){
	$sale_bullock_cart_search=$_POST['query'];
	$stmt = $conn->prepare("SELECT * FROM customers WHERE type='Bullock Cart' AND name LIKE CONCAT(?, '%') OR address LIKE CONCAT(?, '%') OR phone LIKE CONCAT(?, '%') ");
	$stmt->bind_param("sss", $sale_bullock_cart_search, $sale_bullock_cart_search, $sale_bullock_cart_search);
}
else{
	$stmt = $conn->prepare("SELECT * FROM customers WHERE type='Bullock Cart' ");	
}
	$stmt->execute();	
