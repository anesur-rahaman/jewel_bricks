...
if(isset($_POST['query'])){
	$sale_general_search=$_POST['query'];
	$stmt = $conn->prepare("SELECT * FROM customers WHERE type='General' AND name LIKE CONCAT(?, '%') OR address LIKE CONCAT(?, '%') OR phone LIKE CONCAT(?, '%') ");
	$stmt->bind_param("sss", $sale_general_search, $sale_general_search, $sale_general_search);
}
else{
	$stmt = $conn->prepare("SELECT * FROM customers WHERE type='General' ");	
}
	$stmt->execute();	
