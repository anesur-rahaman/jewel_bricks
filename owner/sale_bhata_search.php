...
if(isset($_POST['query'])){
	$sale_bhata_search=$_POST['query'];
	$stmt = $conn->prepare("SELECT * FROM customers WHERE type='Bhatta' AND name LIKE CONCAT(?, '%') OR address LIKE CONCAT(?, '%') OR phone LIKE CONCAT(?, '%') ");
	$stmt->bind_param("sss", $sale_bhata_search, $sale_bhata_search, $sale_bhata_search);
}
else{
	$stmt = $conn->prepare("SELECT * FROM customers WHERE type='Bhatta' ");	
}
	$stmt->execute();	
