...
if(isset($_POST['query'])){
	$sale_contractor_search=$_POST['query'];
	$stmt = $conn->prepare("SELECT * FROM customers WHERE type='Contractor' AND name LIKE CONCAT(?, '%') OR address LIKE CONCAT(?, '%') OR phone LIKE CONCAT(?, '%') ");
	$stmt->bind_param("sss", $sale_contractor_search, $sale_contractor_search, $sale_contractor_search);
}
else{
	$stmt = $conn->prepare("SELECT * FROM customers WHERE type='Contractor' ");	
}
	$stmt->execute();	
