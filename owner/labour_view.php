...
<?php
$id =$_REQUEST['id'];
$result = mysqli_query($conn, "SELECT SUM(advance) as sum_advance, SUM(wages) as sum_wages FROM labour_details WHERE labour_id='$labour_id'");
while($row = mysqli_fetch_array($result))
{
	$advance = $row['sum_advance']-$row['sum_wages'];
}
?>
...
<?php
  ...
  $Color = "red";
  $result_running_advance = mysqli_query($conn, "SELECT SUM(advance) AS sum_advance, SUM(wages) AS sum_wages FROM labour_details where labour_id='$id' AND date <= '$temp_date' ORDER BY id DESC Limit $start, $perpage");
  while($rows_advance=mysqli_fetch_array($result_running_advance)){
  $running_advance = $rows_advance['sum_advance'] - $rows_advance['sum_wages'];
  }  
...
<td>
<button><a rel='facebox' href="labour_details_modify.php?id=<?=$row['id']?>&labour_id=<?=$row['labour_id']?>" <i class="far fa fa-address-card-o"></i>Modify</a></button>
<button><a rel='facebox' href="labour_details_delete.php?id=<?=$row['id']?>&labour_id=<?=$row['labour_id']?>" <i class="far fa fa-address-card-o"></i><font size="" color="red">Delete</font></a></button>
</td>
...
<?php
	...
	{
	$j = $page - 1;
	echo "<span>&nbsp;<button><a id='page_a_link' href='labour_view.php?page=$j&id=$id'>< Prev</a>&nbsp;</button></span>";
	}
	for($i=1; $i <= $totalPages; $i++)
	{
	if($i<>$page)
	{
	echo "<span>&nbsp;<button><a id='page_a_link' href='labour_view.php?page=$i&id=$id'>$i</a>&nbsp;</button></span>";
	}
	else
	{
	...
	{
	$j = $page + 1;
	echo "<span>&nbsp;<button><a id='page_a_link' href='labour_view.php?page=$j&id=$id'>Next</a></button></span>";
	}
	}
?></td>
<td></td>
