...
<label for="sale_bhata_search" class="font-weight-bold lead text-dark"><font size="" color="red">Bhatta</font></label>&nbsp;&nbsp;&nbsp;&nbsp;
<input type="text" name="sale_bhata_search" id="sale_bhata_search" class="form-control form-control-lg rounded-0 border-primary" placeholder="Search by Name or Address or Phone...">
...
&nbsp;&nbsp;&nbsp;&nbsp;<b>OR</b>&nbsp;&nbsp;&nbsp;&nbsp;
<td align ="right"><a href="sale_dues_print_by_type.php?type=<?='Bhatta'?>" class="btn btn-success">Print Dues</a></td>
</div>

<?php 
$stmt = $conn->prepare("SELECT * FROM customers WHERE type='Bhatta' ORDER BY name");
$stmt->execute();
...
<?php        
    // selecting rows
	$sql = "SELECT * FROM customers WHERE type='Bhatta' ";
    if( isset($_GET['char']) ){
...
$(document).ready(function(){
	$("#sale_bhata_search").keyup(function(){
		var sale_bhata_search = $(this).val();
		$.ajax({
		url: 'sale_bhata_search.php',
		method: 'post',
		data: {query:sale_bhata_search},
