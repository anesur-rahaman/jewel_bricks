...
<html lang="en">
<head>
<title>Sale - Bullock Cart (গরুর গাড়ি)</title>
</head>
<body class="main">
<div class="a">
...
<label for="sale_bullock_cart_search" class="font-weight-bold lead text-dark"><font size="" color="red">Bullock Cart (গরুর গাড়ি)</font></label>&nbsp;&nbsp;&nbsp;&nbsp;
<input type="text" name="sale_bullock_cart_search" id="sale_bullock_cart_search" class="form-control form-control-lg rounded-0 border-primary" placeholder="Search by Name or Address or Phone...">
...
&nbsp;&nbsp;&nbsp;&nbsp;<b>OR</b>&nbsp;&nbsp;&nbsp;&nbsp;
<td align ="right"><a href="sale_dues_print_by_type.php?type=<?='Bullock Cart'?>" class="btn btn-success">Print Dues</a></td>
</div>

<?php 
$stmt = $conn->prepare("SELECT * FROM customers WHERE type='Bullock Cart' ORDER BY name");
$stmt->execute();
...
<?php        
    // selecting rows
	$sql = "SELECT * FROM customers WHERE type='Bullock Cart' ";
    if( isset($_GET['char']) ){
...
$(document).ready(function(){
	$("#sale_bullock_cart_search").keyup(function(){
		var sale_bullock_cart_search = $(this).val();
		$.ajax({
		url: 'sale_bullock_cart_search.php',
		method: 'post',
		data: {query:sale_bullock_cart_search},
