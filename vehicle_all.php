...
<?php
$id =$_REQUEST['id'];
$result = mysqli_query($conn, "SELECT SUM(advance) as sum_advance, SUM(salary) as sum_salary, SUM(wages) as sum_wages FROM vehicle_details");
...
<?php
	...
	$sql = "select DISTINCT LEFT(name , 1) as firstCharacter,( select count(*) FROM vehicle where LEFT(name , 1)= firstCharacter ) AS counter FROM vehicle";
...
?>
	...
	<a href="vehicle_details_view.php?id=<?=$row['id']?>" class="btn btn-info btn-sm">View Detials</a></td>
	</td>
	</tr>
<?php
}
